...
// Estrutura de vetor 3D com operações básicas de álgebra vetorial
struct Vec3 {
    float x, y, z;
    ...
    float length() const { return std::sqrt(x*x + y*y + z*z); }              // Módulo do vetor
};

// Estrutura que representa uma bola/esfera
struct Ball {
    Vec3 pos, vel;     // Posição e velocidade
...
// Função que projeta um ponto 3D na tela 2D (perspectiva)
...
// Atualiza a posição da bola com base na sua velocidade (pos = pos + vel * dt)
void updatePosition(Ball& ball) {
    ball.pos = ball.pos + ball.vel * DT;
}

// Trata colisões com as paredes do cubo 3D (inverte a velocidade com CR)
...
// Detecta e resolve colisões entre duas bolas usando conservação de quantidade de movimento
void handleBallCollision(Ball& a, Ball& b) {
    ...
    if (dist < minDist && dist > 0.0f) {
        Vec3 n = delta / dist;                     // Vetor normal unitário
        ...
        if (vel_along_normal > 0) return; // As bolas estão se afastando

        // Fórmula do impulso: j = -(1 + CR) * (v_rel · n) / (1/m1 + 1/m2)
        float m1 = a.mass, m2 = b.mass;
        float j = -(1 + CR) * vel_along_normal / (1 / m1 + 1 / m2);
        Vec3 impulse = n * j; // Vetor impulso

        // Atualização das velocidades com base no impulso (2a Lei de Newton)
        a.vel = a.vel + impulse / m1;  // v1' = v1 + j/m1
        b.vel = b.vel - impulse / m2;  // v2' = v2 - j/m2

        // Correção de sobreposição para evitar "colagem"
        Vec3 correction = n * (minDist - dist) * 0.5f;
        a.pos = a.pos - correction;
        b.pos = b.pos + correction;

        // Altera a cor das bolas para visualização de colisão
...
int main() {
    ...
    while (running) {
        ...
        // Cálculo da energia cinética total: Ec = 1/2 * m * v^2
        float k_total = 0.0f;
        for (auto& b : balls)
        ...
        // Mostra a energia cinética total em tempo real na tela
        std::stringstream ss;
